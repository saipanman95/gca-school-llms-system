CREATE TABLE IF NOT EXISTS `email_address_local` (
...
FROM (
    SELECT
        ...
        COALESCE(NULLIF(TRIM(email.email_address), ''), NULLIF(TRIM(gl.email), '')) AS guardian_email,
        ...
        FROM (
            SELECT
                ...
                ROW_NUMBER() OVER (
                    ...
                    ORDER BY
                        CAST(COALESCE(pea.is_primary_email_address, b'0') AS UNSIGNED) DESC,
                        CASE WHEN pea.email_address_priority_order IS NULL THEN 1 ELSE 0 END,
                        pea.email_address_priority_order,
                        pea.person_email_address_assoc_id
                ) AS email_rank
            FROM person_email_address_assoc_local pea
            LEFT JOIN email_address_local ea
              ON ea.email_address_id = pea.email_address_id
        ) ranked_email
        WHERE email_rank = 1
    ) email
      ON email.person_id = gpa.person_id
) ranked;
